...
func setupWStoreFloat32WithClock(
	chk *sztest.Chk,
	initialTime time.Time, inc ...time.Duration,
) (string, string, *WStoreFloat32) {
	chk.T().Helper()
...
func validateFloat32History(
	...
	chk.Float32Slice(vSlice, expVSlice, 0)
}
...
func Test_WStoreFloat32_UseCase(t *testing.T) {
	...
	chk.Log(
		...
		`parseFloat32: invalid syntax: "abc"`,
		`parseFloat32: invalid syntax: "abc"`+
			`: {{hPath0}}:1 - "{{clkNano0}}|U|key1|abc"`,
		`parseFloat32: invalid range: "1.7E+309"`,
		`parseFloat32: invalid range: "1.7E+309"`+
